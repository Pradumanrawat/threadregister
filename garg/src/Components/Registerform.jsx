...
const Registerform = () => {
  ...
  const validateForm = () => {
    ...
    else if (companyName.length < 3 || companyName.length > 100) newErrors.companyName = 'Company name must be 3-100 characters long'
    //owner/manager name
    if (!ownerName) newErrors.ownerName = 'Owner/Manager Name is required';
    else if (ownerName.length < 3) newErrors.ownerName = 'Owner name must be at least 3 characters long'
    //owner number
    ...
    else if (gstNumber.length != 15) newErrors.gstNumber = 'GST number must be 15 characters long'
    //address
    if (!address) newErrors.address = 'Address is required';
    else if (address.length < 10) newErrors.address = 'Address must be at least 10 characters long'
    //plot number
    if (!plotNumber) newErrors.plotNumber = 'Plot Number is required';
    else if (!Number(plotNumber)) newErrors.plotNumber = 'Plot number must be numeric'
    //sector number
    if (!sector) newErrors.sector = 'Sector is required';
    else if (!Number(sector)) newErrors.plotNumber = 'Plot number must be numeric'
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const response = await axios.post('http://localhost:7000/user', userdata);
